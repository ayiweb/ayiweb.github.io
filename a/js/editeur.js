...
function saveadd() {
  ...
  const aatitle = document.getElementById('aatitle').value.trim();
  const aaimage = document.getElementById('aaimage').value.trim();
  const aatype = document.getElementById('aatype').value.trim();
  const aadetails = document.getElementById('aadetails').value.trim();
  const aapage = document.getElementById('aapage').value.trim();

  if (!category || !aatitle || !aapage) {
      alert('Category, Title, and Page fields are required.');
      return;
  }
  ...
  const dbRef = database.ref(`${category}/${aapage}/`);

  // Set data to the reference
...
// Function to search data in Firebase and populate form fields
function rechercheadd() {
  const category = document.getElementById('category').value.trim();
  const aapage = document.getElementById('aapage').value.trim();

  // Select the form fields
  const image = document.getElementById('aaimage');
  const title = document.getElementById('aatitle');
  const type = document.getElementById('aatype');
  const details = document.getElementById('aadetails');

  if (!category || !aapage) {
      alert('Category and Page fields are required.');
      return;
  }
  ...
  dbRef.once('value', (snapshot) => {
      ...
      if (data) {
          ...
          image.value = data.AAImage || ''; // Use default empty string if data.AAImage is undefined
          title.value = data.AATitle || '';   // Use default empty string if data.AATitle is undefined
          type.value = data.AAType || '';   // Use default empty string if data.AAType is undefined
          details.value = data.AADetails || ''; // Use default empty string if data.AADetails is undefined
      } else {
          alert('No data found for the selected category and page.');
      }
  }, (error) => {
      // Handle errors
...
function updateadd() {
  ...
  dbRef.update(data, (error) => {
      if (error) {
          alert('Data could not be updated. ' + error.message);
      } else {
