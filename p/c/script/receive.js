...
console.log(title); // Should output the value of 'l' parameter (e.g., Informatique)
  ...
  var postsRef = database.ref('COURS/' + encodeURIComponent(title));
...
postsRef.on('value', function(snapshot) {
    ...
    document.getElementById('anglais').innerHTML = '';
    ...
    function displayPosts(posts) {
        document.getElementById('anglais').innerHTML = ''; // Clear existing posts
        ...
        if (hoursDifference >= 24) {
            ...
            document.getElementById('anglais').appendChild(postDiv);
        });
    }
});
...
function btncl(encodedTitle) {
    ...
    window.location.href = `/s/details.html?l=${encodeURIComponent(titleWithUnderscores)}`;
